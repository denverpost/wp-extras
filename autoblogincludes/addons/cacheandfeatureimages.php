	...
	public function check_post_for_images( $post_ID, $ablog, SimplePie_Item $item ) {
		...
		if ( empty( $post_content ) ) {
			...
			if ( $post ) {
                ...
                $this->removeElementsByTagName( 'div', $dom );
                $this->removeElementsByTagName( 'figure', $dom );
                $this->removeElementsByTagName( 'aside', $dom );
                $this->removeElementsByTagName( 'iframe', $dom );
                $dom_content = $dom->saveHTML();
		$dom_content = str_replace( '<meta http-equiv="content-type" content="text/html; charset=utf-8">', '', $dom_content );
                $post_content = preg_replace('~<(?:!DOCTYPE|/?(?:html|head|body))[^>]*>\s*~i', '', $dom_content);
